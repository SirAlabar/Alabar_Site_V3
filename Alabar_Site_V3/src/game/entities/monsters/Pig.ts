/**
 * Pig.ts - Passive creature that drops food when killed
...
export class Pig extends MonsterBase
{
  constructor(assetManager: AssetManager, x: number, y: number)
  {
    const config: MonsterConfig = {
      startX: x,
      startY: y,
      speed: 1.5, // Slightly slower than turkey
      spritesheetKey: 'pig_spritesheet',
      animationPrefix: 'Pig',
      health: 50, // Slightly tankier than turkey
    ...
    // Pigs are passive roaming creatures
   ...
   * Pig only has walk/idle animations (no combat animations)
   */
  ...
  protected onDeath(): void
  {
    ...
    // SiteGame will listen for 'pig-death' event and spawn food
    this.emit('drop-food', {
  ...
  /**
   * Override onHurt - pigs don't have hurt animation
   */
   ...
   * Pigs just wander around randomly
   */
  protected makeAIDecision(delta: number): void
  {
    // Pigs are passive - no chasing or attacking
  ...
  private roam(delta: number): void
  {
    ...
    if (Math.random() < 0.01)
    {
      ...
      // 40% chance to stop and idle (pigs lazier than turkeys)
      if (Math.random() < 0.4)
      {
  ...
  /**
   * Override collision radius for Pig
   */
  getCollisionRadius(): number
  {
    return 28; // Slightly larger than turkey (pigs are chonky)
  }
}